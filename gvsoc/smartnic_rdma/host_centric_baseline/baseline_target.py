...
from rtl_mcu_comp import RtlMcu
...
class BaselineSoc(st.Component):
    ...
    def __init__(self, parent, name, parser=None):
        ...
        if parser is not None:
            ...
            parser.add_argument("--use-mcu", dest="use_mcu", action="store_true",
                help="Enable the Phase 2 RTL MCU path (cv32e40x dummy via Verilated DPI bridge)")
            parser.add_argument("--mcu-binary", dest="mcu_binary", type=str, default=None,
                help="RV32 firmware ELF to preload into MCU memory (Phase 3 real core)")
            args, __ = parser.parse_known_args()
            host_cpu_binary = args.host_binary
            host_cpu_isa = args.host_isa
            host_calibration_profile = args.host_calibration_profile
            use_mcu = args.use_mcu
            mcu_binary = args.mcu_binary
            if mcu_binary is not None:
                use_mcu = True   # --mcu-binary implies --use-mcu

        ico = router.Router(self, "ico")

        use_host_firmware = host_cpu_binary is not None

        # RtlMcu is always instantiated so that dpi_mcu_wrapper.cpp (and its
        # Verilated companion sources, when present) get compiled during every
        # build pass.  behavioral_enabled=True activates the Verilated model at
        # runtime; when False the component is dormant and sends no IoReqs.
        self.rtl_mcu = RtlMcu(self, "rtl_mcu", behavioral_enabled=use_mcu)

        self.host = None
        if not use_mcu and not use_host_firmware:
        ...
        if use_mcu:
            self.rtl_mcu.o_MEM(ico.i_INPUT())
        if mcu_binary is not None:
            self.mcu_loader = utils.loader.loader.ElfLoader(
                self,
                "mcu_loader",
                binary=mcu_binary,
            )
            self.mcu_loader.o_OUT(ico.i_INPUT())
            # START fires after the ELF is written → ensures firmware is in memory
            # before the core begins fetching.
            self.mcu_loader.o_START(self.rtl_mcu.i_FETCHEN())
            self.mcu_loader.o_ENTRY(self.rtl_mcu.i_ENTRY())
